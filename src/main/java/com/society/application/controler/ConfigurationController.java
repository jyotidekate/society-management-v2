...
public class ConfigurationController {
	...
	@Autowired RelativeRelationMasterRepo relationMasterRepo;
	
	@Autowired CasteMasterRepo casteMasterRepo;
	
	@Autowired CategoryMasterRepo categoryMasterRepo;
	...
	@ResponseBody
	public CompanyMaster getConmapnyDetails(@ModelAttribute("user") UserMaster login, Model model, HttpSession session) {
	...
	public ResponseEntity<String> updateCompanyDetails(
			...
			@RequestParam("shareValue") String shareValue,
			@RequestParam("address") String address,
			@RequestParam("state") String state,
			@RequestParam("pinCode") String pinCode,
			@RequestParam("email") String email,
			@RequestParam("authorizedcapital") String authorizedcapital,
			@RequestParam("paidup") String paidup,
			@RequestParam("landLine") String landLine,
			@RequestParam("mobile") String mobile,
			@RequestParam("tDSWithPAN") String tDSWithPAN,
			@RequestParam("tDSWithoutPAN") String tDSWithoutPAN,
	...
	public String saveRelativerelation(@ModelAttribute("Relationrelative") RelativeRelationMaster relationMaster, Model model) {
	...
	public ResponseEntity<String> uploadLogoAndSignature(@RequestParam("module") String module,@RequestParam("type") String type,@RequestParam("file") MultipartFile file,HttpSession session ) {
		try {
			String userId = (String)session.getAttribute("loggedInUserName");
	...
	/*
	 * @PostMapping("/saveDirectorMaster") public String
	 * saveDirectorMaster(@ModelAttribute("saveDirectorMaster") DirectorMaster
	 * directorMaster, Model model) { directorMasterRepo.save(directorMaster);
	 * return "configuration/DirectorDetail"; }
	 */
	...
	public int saveDirectorMaster(@RequestParam(value = "filetag", required = false) MultipartFile file,
            ...
            @RequestParam("dinNoPromoterNo") String dinNoPromoterNo,
            ...
            @RequestParam("shareAmount") String shareAmount,
            @RequestParam("noOfShare") String noOfShare,
		...
		String fileName2 = fileStorageService.storeFile(file2);
		  ...
		  dm.setType(type); dm.setType(branchName); dm.setType(prefix);
		  dm.setType(directorName); dm.setType(dateOfBirth);
		  dm.setType(dinNoPromoterNo); dm.setType(appointmentDate);
		  dm.setType(relativeName); dm.setType(address); dm.setType(district);
		  dm.setType(state); dm.setType(pin); dm.setType(aadharno); dm.setType(panNo);
		  dm.setType(mobileNo); dm.setType(emailId); dm.setType(shareNominalValue);
		  dm.setType(shareAmount); dm.setType(noOfShare);
